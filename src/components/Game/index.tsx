...
function shuffle(
  a: Array<{ image: string; name: string }>
): Array<{ image: string; name: string }> {
...
const Game: React.FC<Props> = ({ data }) => {
  ...
  const handleClick = useCallback(
    (data: { image: string; name: string; index: number }) => {
  ...
  const selectedIndex = useMemo(
    () => selection.map(selected => selected.index),
    [selection]
  );
  ...
  return (
    <Container>
      ...
      {endGame && (
        <>
          <EndGame width={window.innerWidth} height={window.innerHeight}>
